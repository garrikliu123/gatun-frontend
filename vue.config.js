...
checkoutRouter.get("/success", function(req, res) {
    ...
    axios
        .get("http://localhost:8080/checkout/checkSession", {
            params: {
    ...
    res.redirect(`http://localhost:8080/checkout?checkout_session_id=${params.checkout_session_id}`);
});
...
checkoutRouter.get("/checkSession", async function(req, res) {
    ...
    try {
        if (params && params.sessionId) {
            await stripe.checkout.sessions.retrieve(params.sessionId, function(err, session) {
                if (err) {
                    res.send({
                        success: false,
                        errMsg: err.raw.message
